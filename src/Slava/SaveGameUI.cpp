...
slava::SaveGameUI::SaveGameUI(sf::Font& font, std::string path) {
    std::string saveGameName="Slot #0";
...
void slava::SaveGameUI::control(GameWorld* world) {
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
		...
		for(int i=0;i<backShapes.size();i++) if(contains(backShapes[i],xM,yM)){
            world->getMainCharacter()->exportCharStats("savegames.txt",i);
            this->active=false;
            world->unpause();
        }
    }
    if(sf::Mouse::isButtonPressed(sf::Mouse::Right)){
        if (contains(backShape, xM, yM)) {
            this->active=false;
            world->getUI("pauseUI")->active=true;
            /*
            for(auto i:world->views){
                if(compareStr(i.first,"pauseUI"))i.second->active=true;
            }*/
		}
    }
...
void slava::SaveGameUI::draw(sf::RenderWindow& win) {
